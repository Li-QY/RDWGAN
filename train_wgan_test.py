...
from model_wgan_Dense import Generator_RDN, Discriminator
...
parser.add_argument('--nDenselayer', default=8, type=int, help='Number of Denselayers')
parser.add_argument('--nBlock', default=3, type=int, help='Number of Dense Blocks')
...
if __name__ == '__main__':
    ...
    netG = Generator_RDN(UPSCALE_FACTOR, 3, GROWTHRATE, DENSELAYERS, DENSEBLOCKS)
    ...
    for epoch in tqdm(range(1, NUM_EPOCHS + 1), desc="Epoch"):
        ...
        for iteration, datas in tqdm(enumerate(train_loader, 1),
            ...
            gradient_penalty_d = compute_gradient_penalty(device, netD, real_img, fake_img)
            # Modification 3: remove log(adv) 
            d_loss = - real_out + fake_out + gradient_penalty_d*10
...
#             , tv_loss + tv_loss 
            image_loss, adversarial_loss, perception_loss = generator_criterion(fake_out, fake_img, real_img)
            gradient_penalty_g = compute_gradient_penalty(device, netD, real_img, fake_img)
            g_loss = image_loss + adversarial_loss+  perception_loss + gradient_penalty_g*10
            g_loss.backward()
            ...
            writer.add_scalar("Loss/WGAN-gp-d", gradient_penalty_d.item(), step)
            writer.add_scalar("Loss/WGAN-gp-g", gradient_penalty_g.item(), step)
...
#             writer.add_scalar("Loss/tv_loss", tv_loss.item(), step)
        ...
        if torch.cuda.is_available():
            ...
            if epoch%5 == 0:
                ...
                torch.save(optimizerG.state_dict(), out_path + '/optimizerG_epoch_%d_%d.pth' % (UPSCALE_FACTOR,epoch))
                torch.save(optimizerD.state_dict(), out_path + '/optimizerD_epoch_%d_%d.pth' % (UPSCALE_FACTOR,epoch))
        else:
            torch.save(netG.state_dict(), out_path + '/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR,epoch))
            if epoch%5 == 0:
                torch.save(netD.state_dict(), out_path + '/netD_epoch_%d_%d.pth' % (epoch))
                torch.save(optimizerG.state_dict(), out_path + '/optimizerG_epoch_%d_%d.pth' % (UPSCALE_FACTOR,epoch))
                torch.save(optimizerD.state_dict(), out_path + '/optimizerD_epoch_%d_%d.pth' % (UPSCALE_FACTOR,epoch))
